<html lang="en">
<head>
      <meta name="description" content="yeah i'm a pokemon master suck it">
              <meta name="robots" content="noai, noimageai">

<title>MASTERBALL / HOME</title>
<meta name="viewport" content="initial-scale=1.0">

<link rel="shortcut icon" type="image/x-icon" href="mball.ico" />

<style type="text/css">

@font-face {
    font-family: Futura;
    src: url('http://kingdra.net/x/futura.ttf') format(futura.ttf);
}

body{
    background: #eaeaea;
    margin: 0;
    font: normal 100%/140% 'Futura';
    color: #333;
}

#container{
    width: 40vw;
    margin: 5em auto;
}

img.bull{
    
}

.item{
    background: #ffffff;
      line-height: 140%;
      padding: 10px 5px 5px 5px;
}

nav .item{
    padding: 10px;
  font-size: 160%;
        line-height: 180%;

}

nav .item a:link, nav .item a:visited{
    text-transform: uppercase;
  text-decoration: none;
}

nav .item a.trainer:link, nav .item a.trainer:visited{
      color: #82b1ca;
}

nav .item a.gym:link, nav .item a.gym:visited{
      color: #999;
}

nav .item a.tcg:link, nav .item a.tcg:visited{
      color: #0A4A87;
}

nav .item a.pokerogue:link, nav .item a.pokerogue:visited{
      color: #346485;
}

nav .item a.map:link, nav .item a.map:visited{
      color: #72A014;
}

nav .item a:hover, nav .item a:focus{
    color: #f30000!important;
}

img.navpng{
    padding-right: 5px;
    width:25px;
}

.tooltip {
    position: relative;
    display: inline-block;
}

.tooltip .tooltiptext {
    display: none;
    background-color: #7E5FC6;
    border: 4px solid #F2B3F4;
    color: #fff;
    text-align: center;
    padding: 5px;
    position: absolute;
    z-index: 1;
    bottom: -30px;
    right: 0%;
    margin-left: -100px;
    font-size: 70%;
    text-transform: none!important;
    line-height: 110%;
}

a.tooltip:hover{
    cursor: pointer;
}

.tooltip:hover .tooltiptext{
    display: block;
}

.footer{
    text-align: center;
    color: #999;
}

.footer a:link, .footer a:visited{
    color: #999;
}

.footer a:hover, .footer a:focus{
    color: #333;
}


</style>

</head>

<body>

<div id="container">

    <nav>
        <span class="item"><a href="trainer" rel="me" class="trainer"><img src="trainer.png" class="navpng"> Trainer</a> <img src="revive.png" class="bull"> an archive of all my pok&eacute;mon</span>
        <p>&nbsp;</p>
        <span class="item"><a href="gym" rel="me" class="gym"><img src="gymbol.png" class="navpng">Gym</a> <img src="revive.png" class="bull"> site information, exits, & pok&eacute;go</span>
        <p>&nbsp;</p>
        <span class="item"><a href="" rel="me" class="tcg tooltip"><img src="tcg.png" class="navpng">TCG <span class="tooltiptext">upcoming! :)</span></a> <img src="revive.png" class="bull"> my addiction to the mobile game </span>
        <p>&nbsp;</p>
        <span class="item"><a href="" rel="me" class="pokerogue tooltip"><img src="pokerogue.png" class="navpng">Pok&eacute;Rogue<span class="tooltiptext">upcoming! :)</span></a> <img src="revive.png" class="bull"> an archive of my runs</span>
                <p>&nbsp;</p>

        <span class="item"><a href="map" rel="me" class="map"><img src="map.png" class="navpng">Map</a> <img src="revive.png" class="bull"> shows your present location </span>

    </nav>

</div>

    <div class="footer">
        &copy; 2017-2024 masterball/icirr.us is a <a href="https://kingdra.net">kingdra.net</a> project. for further information, see <a href="" class="tooltip">documentation<span class="tooltiptext">upcoming! :)</span></a>. <a href="/">clear?</a>
    </div>
</body>
</html>